...
class _PosScreenState extends State<PosScreen> {
  late List<PosProduct> _allProducts;
  ...
  void _initializeProducts() {
    final categories = ['Sweets', 'Beverages', 'Snacks', 'Bakery'];
    final categoryImages = {
      'Sweets': ['chocolate', 'cake', 'candy', 'donut'],
      'Beverages': ['juice', 'coffee', 'soda', 'tea'],
      'Snacks': ['chips', 'nuts', 'popcorn', 'pretzel'],
      'Bakery': ['bread', 'croissant', 'bagel', 'muffin'],
    };
    final units = ['kg', 'g', 'L', 'pc'];

    _allProducts = List.generate(50, (index) {
      final category = categories[index % 4];
      final productType = categoryImages[category]![index % 4];
      final unit = units[index % 4];

      return PosProduct(
        id: 'prod_$index',
        name: '$productType ${index + 1}',
        category: category,
        price: 100.0 + Random().nextInt(1500),
        imageUrl:
            'https://source.unsplash.com/random/300x300/?$productType&$index',
        unit: unit,
      );
    });
  }
  ...
  double _calculateGrandTotal() {
    ...
    double itemDiscounts =
        _cartItems.fold(0, (sum, item) => sum + item.discount);
    double totalAfterItemDiscounts = subtotal - itemDiscounts;
    double tax = (totalAfterItemDiscounts - _overallDiscount) * 0.00;
  ...
  Widget _buildCartView(ThemeData theme) {
    ...
    return Column(
      ...
      children: [
        ...
        _buildTotalRow('Item Discounts',
            '- Rs. ${itemDiscounts.toStringAsFixed(2)}', theme),
        const Divider(height: 20),
        _buildTotalRow(
            'Grand Total', 'Rs. ${grandTotal.toStringAsFixed(2)}', theme,
            isBold: true),
        const SizedBox(height: 20),
        Row(
          children: [
            ...
            Expanded(
                child: ElevatedButton(
                    ...
                    onPressed: _cartItems.isNotEmpty
                        ? () => setState(() => _isFinalizingPayment = true)
                        : null,
  ...
  Widget _buildPaymentView(ThemeData theme) {
    ...
    double itemDiscounts =
        _cartItems.fold(0, (sum, item) => sum + item.discount);
    final totalAfterItemDiscounts = subtotal - itemDiscounts;

    final overallDiscount =
        double.tryParse(_overallDiscountController.text) ?? 0.0;
    final tax = (totalAfterItemDiscounts - overallDiscount) * 0.00;
    final grandTotal = totalAfterItemDiscounts - overallDiscount + tax;
      ...
      children: [
        ...
        Padding(
          ...
          child: TextField(
            ...
            decoration:
                const InputDecoration(labelText: 'Overall Discount (Rs.)'),
            onChanged: (_) =>
                setState(() {}), // Trigger UI update on discount change
          ),
        ),
        _buildTotalRow('Tax (0%)', 'Rs. ${tax.toStringAsFixed(2)}', theme),
        const Divider(height: 20),
        _buildTotalRow(
            'Grand Total', 'Rs. ${grandTotal.toStringAsFixed(2)}', theme,
            isBold: true),
          ...
          children: [
            ...
            Expanded(
                child: _buildPaymentMethodChip(
                    'Credit', Icons.credit_card, theme)),
          ],
        ),
        if (_paymentMethod == 'Cash') ...[
          ...
          TextField(
            ...
            onChanged: (_) => setState(() {
              ...
              _changeDue = tendered - grandTotal;
            }),
          ),
          const SizedBox(height: 10),
          Text(
            ...
            style: TextStyle(
              fontSize: 18,
              color: _changeDue < 0 ? Colors.red : Colors.green,
              fontWeight: FontWeight.bold,
            ),
          ),
        ],
  ...
  Widget _buildPaymentMethodChip(
      String method, IconData icon, ThemeData theme) {
          ...
          Row(
            children: [
              IconButton(
                ...
                onPressed: () => setState(() {
                  if (item.quantity > (item.product.unit == 'pc' ? 1 : 0.1)) {
                    item.quantity =
                        (item.quantity - (item.product.unit == 'pc' ? 1 : 0.1))
                            .clamp(item.product.unit == 'pc' ? 1 : 0.1,
                                double.infinity);
                  } else {
  ...
  Widget _buildTotalRow(String title, String amount, ThemeData theme,
      {bool isBold = false}) {
    final style =
  ...
  void _editItemQuantity(CartItem item) {
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          title: Text('Edit ${item.product.name} Quantity'),
          content: Column(
                ...
                Row(
                  ...
                  children: [
                    IconButton(
                      ...
                      onPressed: () {
                        final current =
                            double.tryParse(quantityController.text) ?? 0.1;
                        quantityController.text =
                            (current - 0.1).toStringAsFixed(1);
                      },
                    ),
                    IconButton(
